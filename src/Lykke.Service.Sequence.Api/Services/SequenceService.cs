...
namespace Lykke.Service.Sequence.Api.Services
{
    public class SequenceService : ISequenceService
    {
        ...
        private readonly ISequenceInsightClient _sequenceInsightClient;
        ...
        {
            var sendAmount = Money.FromUnit(amount, Asset.Sequence.Unit);
            var txsUnspent = await _sequenceInsightClient.GetTxsUnspentAsync(fromAddress.ToString(), _minConfirmations);
            ...
            var feeMoney = Money.FromUnit(_fee, Asset.Sequence.Unit);
        ...
        public async Task BroadcastAsync(Transaction transaction, Guid operationId)
        {
            ...
            try
            {
                response = await _sequenceInsightClient.BroadcastTxAsync(transaction.ToHex());
            ...
            {
                await _log.WriteErrorAsync(nameof(SequenceService), nameof(BroadcastAsync),
                    $"transaction: {transaction}, operationId: {operationId}", ex);

                throw;
            }

            var block = await _sequenceInsightClient.GetLatestBlockHeight();
        ...
        {
            return await _sequenceInsightClient.GetBalance(address, _minConfirmations);
        }
        ...
        {
            var txs = await _sequenceInsightClient.GetAddressTxs(fromAddress, continuation);
