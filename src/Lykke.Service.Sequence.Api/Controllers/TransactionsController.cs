...
namespace Lykke.Service.Sequence.Api.Controllers
{
    ...
    public class TransactionsController : Controller
    {
        ...
        public async Task<IActionResult> Build([Required, FromBody] BuildSingleTransactionRequest request)
        {
            ...
            var fromAddress = _sequenceService.GetBitcoinAddress(request.FromAddress);
            if (fromAddress == null)
            {
                return BadRequest(ErrorResponse.Create($"{nameof(request.FromAddress)} is not a valid"));
            }

            var toAddress = _sequenceService.GetBitcoinAddress(request.ToAddress);
            if (toAddress == null)
            {
                return BadRequest(ErrorResponse.Create($"{nameof(request.ToAddress)} is not a valid"));
            }

            if (request.AssetId != Asset.Sequence.Id)
            {
            ...
            var amount = Conversions.CoinsFromContract(request.Amount, Asset.Sequence.Accuracy);
            var fromAddressBalance = await _sequenceService.GetAddressBalance(request.FromAddress);
            var fee = _sequenceService.GetFee();
            ...
            var transactionContext = await _sequenceService.BuildTransactionAsync(request.OperationId, fromAddress, 
        ...
        public async Task<IActionResult> Broadcast([Required, FromBody] BroadcastTransactionRequest request)
        {
            ...
            var broadcast = await _sequenceService.GetBroadcastAsync(request.OperationId);
            if (broadcast != null)
            {
                return new StatusCodeResult(StatusCodes.Status409Conflict);
            }

            var transaction = _sequenceService.GetTransaction(request.SignedTransaction);
            if (transaction == null)
            {
            ...
            await _sequenceService.BroadcastAsync(transaction, request.OperationId);
        ...
        {
            var broadcast = await _sequenceService.GetBroadcastAsync(operationId);
            if (broadcast == null)
            {
                return NoContent();
            }
            ...
            var amount = broadcast.Amount.HasValue ?
                Conversions.CoinsToContract(broadcast.Amount.Value, Asset.Sequence.Accuracy) : "";

            var fee = broadcast.Fee.HasValue ?
                Conversions.CoinsToContract(broadcast.Fee.Value, Asset.Sequence.Accuracy) : "";
        ...
        public async Task<IActionResult> DeleteBroadcast([Required] Guid operationId)
        {
            ...
            await _sequenceService.DeleteBroadcastAsync(broadcast);
        ...
        {
            var sequenceAddress = _sequenceService.GetBitcoinAddress(address);
            if (sequenceAddress == null)
            {
            ...
            [FromQuery] string afterHash)
        {
            ...
            var txs = await _sequenceService.GetFromAddressTxs(address, take, afterHash);
