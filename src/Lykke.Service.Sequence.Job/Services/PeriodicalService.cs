...
using Lykke.Service.Sequence.Api.Services.Helpers;
...
namespace Lykke.Service.Sequence.Job.Services
{
    public class PeriodicalService : IPeriodicalService
    {
        ...
        public async Task UpdateBroadcasts()
        {
            ...
            {
                var tx = await _sequenceInsightClient.GetTx(item.Hash);
        ...
        {
            var balance = await _sequenceInsightClient.GetBalance(address, _minConfirmations);
